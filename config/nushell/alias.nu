...
def "update" [] {
    nix flake update ~/desktop-nixos/ 
}
...
def "rebuild" [cmd: string] {
    s -v
    sec dec
    s nixos-rebuild $cmd --flake $"($env.HOME)/desktop-nixos/" --impure
    sec rm
}

def "shell" [...pkgs: string] {
    let pkglist = ($pkgs | each { |p| $"nixpkgs#($p)" })
    nix shell -I ~/desktop-nixos ...$pkglist 
}
...
def "sec enc" [] {
    s -v
    echo (key) | openssl aes-256-cbc -salt -pbkdf2 -e -in /tmp/secrets.toml -out ~/desktop-nixos/secrets -pass stdin
}

def "sec dec" [] {
    s -v
    echo (key) | openssl aes-256-cbc -salt -pbkdf2 -d -in ~/desktop-nixos/secrets -out /tmp/secrets.toml -pass stdin
}
