...
def validate_p3(bundle):
    ...
    if result["version"]!=4 or result["status"]!="passed" or result["original_failure_rc"]!=0 or result["manifest_sha256"]!=manifest_envelope["manifest_sha256"] or any(not HEX64.fullmatch(result[key] or "") for key in hashes):raise Drift("P3 result differs")
