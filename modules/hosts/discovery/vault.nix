    ...
    systemd.services.vault-agent = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${pkgs.openbao}/bin/bao agent -config=${pkgs.writeText "vault-agent.hcl" ''
          ...
          # P3.3 harbor (special-cased): harbor-setup.sh reads these 2 from this
          # render instead of the sops .env. The other harbor secrets are
          # generated by Harbor's `prepare`, never sourced from env. 0400 —
          # harbor.service runs as root and no rootless-compose user needs it.
          template {
