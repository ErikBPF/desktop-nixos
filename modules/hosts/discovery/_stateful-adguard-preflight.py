...
def plan(inventory):
    ...
    exact(baseline["exporter"],{"reachable","required_family_count","sample_count"},"exporter")
    if baseline["exporter"]["reachable"] is not True or baseline["exporter"]["required_family_count"]!=3 or not nonnegative(baseline["exporter"]["sample_count"]):raise Drift("exporter baseline invalid")
    stable_baseline={"api":{"filtering_enabled":api["filtering_enabled"],"protection_enabled":api["protection_enabled"],"query_log_enabled":api["query_log_enabled"]},"dns":{name:{"answered":probe["answer_count"]>0,"status":probe["status"]} for name,probe in baseline["dns"].items()},"exporter":{"reachable":baseline["exporter"]["reachable"],"required_families_present":baseline["exporter"]["required_family_count"]==3}}
