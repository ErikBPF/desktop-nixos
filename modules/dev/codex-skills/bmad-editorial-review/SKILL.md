...
- `{workflow.<name>}` resolves to fields in `customize.toml`'s `[workflow]` table (overrides win per the structural merge rules).
...
1. Read `{skill-root}/customize.toml` directly; apply project override files if present. Execute each `{workflow.activation_steps_prepend}` entry in order, and hold `{workflow.persistent_facts}` as standing context for the session.
