...
A guided authoring flow that turns an idea — a themed cast, a one-off persona, or a pile of raw profile data — into custom party members and groups, written to the project's override TOML. The output is configuration; you do the actual write (see *Write the override*).
...
**First, check for code collisions.** A custom member whose `code` matches an existing collective member silently *overrides* it. Before composing, resolve the collective once — `uv run {skill-root}/scripts/resolve_party.py --project-root {project-root} --skill {skill-root}` — and check each new member's `code` against the returned members. On a collision, surface it ("`analyst` would override the existing Analyst — intended, or pick a different code?") and let the user confirm or rename. One check, not a gate.

Compose the sparse override, show it to the user, wait for an explicit yes, then write it yourself to `{project-root}/.agents/party-mode.user.toml` (default) or `.agents/party-mode.toml` when the party is meant to be shared. Exact entries only: the `party_members` tables, any `party_groups` table (including its `memory` flag), and `default_party` if the user opted in. Keep it sparse — only the new entries, never a copy of the base customize.toml. Never edit the skill's own `customize.toml`: it's HM-managed and read-only at runtime.
