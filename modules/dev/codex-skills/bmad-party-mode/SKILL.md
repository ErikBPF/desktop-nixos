...
- **Scripts** (run via `uv run`): `{skill-root}/scripts/resolve_party.py` resolves the roster, `party_mode`, `memory_enabled`, and scene/`open_cast` from `{skill-root}/customize.toml` (no `_bmad` install required on this machine; it degrades gracefully). Party memory is a plain append to the memlog file.
