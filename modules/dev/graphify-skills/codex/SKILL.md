...
> **graphify needs no API key. Never ask the user for one, and never block on one.** Supported code is extracted structurally (AST) with no LLM and no key at all — a supported-code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (docs, papers, images, and R files without an AST extractor) uses Gemini **only if** `GEMINI_API_KEY`/`GOOGLE_API_KEY` is already set; otherwise the host agent itself is the LLM. graphify does **not** read `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or any other provider key. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** check whether `GEMINI_API_KEY` or `GOOGLE_API_KEY` is set. If neither is set and `GRAPHIFY_NO_TIPS` is unset, print the one-line Gemini setup tip once; when `GRAPHIFY_NO_TIPS` is set, continue without a setup tip or warning. If either key is set, use `graphify.llm.extract_corpus_parallel(files, backend="gemini")` for semantic extraction instead of dispatching subagents. The default Gemini model is `gemini-3-flash-preview`; set `GRAPHIFY_GEMINI_MODEL` or pass `--model` in headless CLI flows to override it.
...
**Fast path:** If detection found zero docs, papers, images, and R files (supported-code-only corpus), skip Part B entirely and go straight to Part C. AST handles the code - there is nothing for semantic subagents to do. **First write an empty semantic file** so Part C's merge has its input (it reads `.graphify_semantic.json` unconditionally; without this a code-only run hits `FileNotFoundError`):
...
detect = json.loads(Path('graphify-out/.graphify_detect.json').read_text(encoding=\"utf-8\"))
# Content files and R files go to semantic extraction. Supported code is already
# covered structurally by Part A; R currently has no AST extractor. Video is
# transcribed to a document in Step 2.5 first.
all_files = [f for cat in ('document', 'paper', 'image') for f in detect['files'].get(cat, [])]
all_files += [f for f in detect['files'].get('code', []) if Path(f).suffix.lower() == '.r']
...
> **Codex platform:** Uses `spawn_agent` + `wait_agent` instead of the Agent tool.
...
> Pin semantic extraction workers to `gpt-5.6-terra` with `fork_turns="none"`; use a unique `task_name` per chunk.
...
```
spawn_agent(task_name="graphify_chunk_NN", model="gpt-5.6-terra", fork_turns="none", message="Your task is to perform the following. Follow the instructions below exactly.\n\n<agent-instructions>\n[extraction prompt, with FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, DEEP_MODE substituted]\n</agent-instructions>\n\nExecute this now. Output ONLY the structured JSON response.")
```

After all agents are dispatched, wait for completion notifications and collect each final result in memory:
```
wait_agent(timeout_ms=3600000)   # repeat until every graphify_chunk_NN completes
```
...
Wait for all subagents. On Codex, validate each in-memory result as described in Step B2, then continue with the cache/save logic below; the disk-file checks apply only to hosts whose agents write chunk files. On those hosts, for each result:
