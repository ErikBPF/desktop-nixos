      ...
      on =
        ...
        ++ map (event: {
          _args = [
            ...
            (lib.generators.mkLuaInline ''
              function(m)
                local description = "Samsung Electric Company QBQ90 0x01000E00"
                if m ~= nil and m.description ~= description then
                  return
                end
                hl.timer(function()
                  local target = hl.get_monitor("desc:" .. description)
                  if target == nil then
                    return
                  end
                  for id = 1, 9 do
                    local workspace = hl.get_workspace(id)
                    if workspace ~= nil and (workspace.monitor == nil or workspace.monitor.id ~= target.id) then
                      hl.dispatch(hl.dsp.workspace.move({ workspace = workspace, monitor = target }))
                    end
                  end
                end, { timeout = ${toString hotplugRecoveryDelayMs}, type = "oneshot" })
              end
            '')
          ];
        }) ["hyprland.start" "monitor.added"];
