...
if cmd=='date':
 if '-d' in args:
  ...
  count=int((p/'clock').read_text()) if (p/'clock').exists() else 0
  (p/'clock').write_text(str(count+1))
  print(2000000000+(172800 if mode=='advances' and count else 0))
elif cmd=='ss':
 ...
 if args[0]=='snapshots':
  ...
  if args != ['ls','--json','a'*64,'/var/lib/vault-snapshots/openbao.snap']: sys.exit(1)
  ...
  if mode=='noncanonical': node['mtime']='@1999999940'
  print(json.dumps(node))
 elif args[0]=='dump':
  if args != ['dump','a'*64,'/var/lib/vault-snapshots/openbao.snap']: sys.exit(1)
  if mode=='interrupt-dump':
   (p/'blocked').touch(); time.sleep(120)
...
class RestoreTest(unittest.TestCase):
    def run_case(self, mode):
        with tempfile.TemporaryDirectory() as directory:
            ...
            script=script.replace('source_path=${snapFile}', 'source_path='+SNAP)
            ...
            env=dict(os.environ,PATH=str(p/'bin')+':'+os.environ['PATH'],CASE_ROOT=str(p),CASE_MODE=mode,REAL_DATE=shutil.which('date'))
            if mode.startswith('interrupt'):
                process=subprocess.Popen(['bash','-c',script],env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,start_new_session=True)
                try:
                    deadline=time.monotonic()+5
                    while not (p/'blocked').exists() and process.poll() is None and time.monotonic()<deadline:
                        time.sleep(.02)
                    self.assertTrue((p/'blocked').exists(), 'did not reach interruption stage')
            ...
            self.assertFalse(any('synthetic-secret' in arg for call in calls for arg in call))
            self.assertEqual(list((p/'scratch').iterdir()),[])
            if (p/'pid').exists():
                with self.assertRaises(ProcessLookupError): os.kill(int((p/'pid').read_text()),0)
            if mode=='ok':
                self.assertEqual(result.returncode,0,result.stderr)
                self.assertIn(['restic','snapshots','--json','--path',SNAP,'--latest','1'],calls)
                self.assertEqual(sum(c[:2]==['restic','snapshots'] for c in calls),1)
                self.assertIn(['restic','dump',ID,SNAP],calls)
                self.assertIn(ID,result.stdout)
                self.assertIn('voyager',result.stdout)
                self.assertIn('source_age_seconds=60',result.stdout)
                ...
                if mode not in ['restore','unseal','auth','advances','interrupt-child']:
                    self.assertFalse(any(c[:3]==['bao','operator','init'] for c in calls))

    def test_remote_success(self): self.run_case('ok')
    def test_fail_closed(self):
        for mode in ['unavailable','empty','ambiguous','badid','wrongpath','symlink','zero','badtime','stale','boundary','future','partial','restore','unseal','auth','occupied','dead','noncanonical','advances','interrupt-dump','interrupt-child']:
