...
class TransitionTest(unittest.TestCase):
    ...
    def test_exact_revision_channel_is_bound_before_recreate(self):
        ...
        self.assertEqual(manifest["recovery_commands"][0],["sudo","-u","erik","--","servarr-exact-revision","activate","rollback","--prefetch",self.revisions["prefetch"]["path"],"--authorization",self.t.LAYOUT["revision_rollback_authorization"],"--output",self.t.BASE+"/rollback-revision.json"]);self.assertEqual(manifest["recovery_render_sha256"],self.revisions["rollback"]["render_sha256"])
    ...
    def test_actual_p3_schema_cross_links_and_drift_fail_closed(self):
        ...
        for mutate in (lambda x:x["result"].update(status="failed"),lambda x:x["result"].update(core_row_count=23),lambda x:x["result"]["postrestore_evidence"].update(status="partial"),lambda x:x["manifest_envelope"]["manifest"].update(resources=["adguard"]),lambda x:x["observation"].update(version=4)):
