...
**Status:** GPUs exchanged; host, storage, network and bounded Apollo compute acceptance passed. The later two-card GPU preparation supersedes boot retention; household inference remains paused.

See [GPU preparation](apollo-gpu-preparation.md) for the two RTX 5060 Ti cards
and the open-module/power-policy acceptance. The three-entry
