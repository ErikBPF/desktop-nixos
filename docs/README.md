...
| [`proposals/2026-07-02-tokensave-dataplatform-eval.md`](proposals/2026-07-02-tokensave-dataplatform-eval.md) | Enabled for eval (2026-07-02) — TokenSave semantic-code-graph MCP on `dataplatform-airflow`/`-spark`, delivered via gitignored `devenv.local.nix` (no global install, worldwide counter off, zero tracked footprint). Graphs built; A/B benchmark plan + exit criteria pending a real work session. Headroom deferred (RTK overlap). |
