...
| [`proposals/2026-06-30-deploy-rs-as-deploy-standard.md`](proposals/2026-06-30-deploy-rs-as-deploy-standard.md) | Draft / Proposed — adopt deploy-rs (magic rollback) as the standard for *subsequent* remote switches; nixos-anywhere keeps first install. Motivated by the voyager bricking episode. |
| [`proposals/2026-06-30-offsite-dr-crown-jewels.md`](proposals/2026-06-30-offsite-dr-crown-jewels.md) | Proposal (draft) — adopt Oracle `voyager` as the off-premise DR anchor for the crown-jewel config tier: tf state (append-only REST), passphrase-age escrow of the sops age key (closes the single-point-of-loss), GitHub-independent `.env.sops` copy, and the Vault backup RFC's off-site destination. All copies today share one house. |
