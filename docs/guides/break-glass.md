...
| A Voyager deployment loses tailnet connectivity | Before deployment, run `just voyager-rollback-preflight`, open the authenticated OCI serial/web console, and log in as `erik`. From that console, run the printed generation-bound commands; after tailnet returns, run `(cd machines && just check-stack voyager offsite)` from the Servarr checkout and inspect every required container. | The printed generation is active; SSH/tailnet returned; status shows every required `offsite` container running/healthy. |
